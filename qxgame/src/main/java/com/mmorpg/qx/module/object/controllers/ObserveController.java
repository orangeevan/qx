...
import com.mmorpg.qx.module.object.controllers.observer.ActionObserver.ObserverType;
...
public class ObserveController {
	 ...
	 * notify that creature moved
	 */
	protected void notifySeeObservers(AbstractVisibleObject visibleObject) {
		Iterator<ActionObserver> ite = observers.iterator();
		while (ite.hasNext()) {
			ActionObserver observer = ite.next();
			if (observer.isDisposable()) {
	 ...
	 */
	public void notifyAttackObservers(AbstractCreature creature) {
		Iterator<ActionObserver> ite = observers.iterator();
		while (ite.hasNext()) {
			ActionObserver observer = ite.next();
			if (observer.isDisposable()) {
	 ...
	 */
	protected void notifyAttackedObservers(AbstractCreature creature) {
		Iterator<ActionObserver> ite = observers.iterator();
		while (ite.hasNext()) {
			ActionObserver observer = ite.next();
			if (observer.isDisposable()) {
	...
	public void notifyDieObservers(AbstractCreature creature) {
		Iterator<ActionObserver> ite = observers.iterator();
		while (ite.hasNext()) {
			ActionObserver observer = ite.next();
			if (observer.isDisposable()) {
