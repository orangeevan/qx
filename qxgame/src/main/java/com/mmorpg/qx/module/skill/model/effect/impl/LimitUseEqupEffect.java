 ...
 */
public class LimitUseEqupEffect extends AbstractEffectTemplate {
    ...
    public boolean applyEffect(Effect effect) {
        ...
        if (!CollectionUtils.isEmpty(creatures)) {
            for (AbstractCreature creature : creatures) {
                creature.getEffectController().setStatus(EffectStatus.Limit_Use_Equip, false);
            }
            return true;
        }
        return false;
    ...
    public void endEffect(Effect effect, EffectController controller) {
        ...
        if (!CollectionUtils.isEmpty(creatures)) {
            creatures.stream().forEach(creature -> {
                creature.getEffectController().unsetStatus(EffectStatus.Limit_Use_Equip, false);
            });
        }
    }
