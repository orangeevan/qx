...
/**
 * @author wang ke
 * @description:禁止魔物娘投骰子
 * @since 14:47 2020-09-10
 */
public class LimitThrowDiceEffect extends AbstractEffectTemplate {
    ...
    public boolean applyEffect(Effect effect) {
        ...
        if (!CollectionUtils.isEmpty(creatures)) {
            for (AbstractCreature creature : creatures) {
                creature.getEffectController().setStatus(EffectStatus.Limit_Throw_Dice, false);
            }
            return true;
        }
        return false;
    ...
    public void endEffect(Effect effect, EffectController controller) {
        ...
        if (!CollectionUtils.isEmpty(creatures)) {
            creatures.stream().forEach(creature -> {
                creature.getEffectController().unsetStatus(EffectStatus.Limit_Throw_Dice, false);
            });
        }
    }
}
